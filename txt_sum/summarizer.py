    ...
    def _get_llm_provider(self, provider_name: Optional[str] = None) -> BaseLLMProvider:
        ...
        #     provider = OpenAIProvider(settings)
            ...
            raise ValueError(
                ...
                f"Supported providers: lm_studio, qwen"
            )
...
"""
    ...
    def _suggest_filenames_from_content(
        self,
        input_file: Path,
        content_excerpt: str,
        provider: Optional[str] = None
        ...
        if not self.llm_provider or provider:
            try:
                ...
                raise ValueError(f"Failed to initialize LLM provider: {e}") from e
...
"""
        ...
        try:
            ...
            result = self._parse_filename_suggestions(response, input_file, output_file)
            if not result.get("original") or not result.get("output"):
                raise ValueError(f"Failed to parse filename suggestions from LLM response. Response: {response[:200]}")
            return result
        except Exception as e:
            # Re-raise to allow caller to handle with better context
            raise ValueError(f"Failed to generate filename suggestions: {e}") from e
...
"""
    ...
    def _parse_filename_suggestions(
        ...
        try:
            ...
            json_parse_error = f"JSON decode error: {e}"
        except KeyError as e:
            json_parse_error = f"Missing key in JSON: {e}"
        except Exception as e:
            json_parse_error = f"Unexpected error parsing JSON: {e}"
        
        # Fallback: Parse numbered list format
        # Look for "Original file suggestions:" or similar patterns
        # Handle variations: "1. filename", "1) filename", "1 - filename", etc.
